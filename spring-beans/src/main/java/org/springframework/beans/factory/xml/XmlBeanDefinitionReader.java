...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	//表示当前线程已经加载过的EncodedResource 资源
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		//1、将resource 包装成代编码格式的资源
		//2、重载调用 loadBeanDefinitions
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		//拿到当前线程已经加载过的 所有EncodedResource 资源
		Set<EncodedResource> currentResources = this.resourcesCurrentlyBeingLoaded.get();

		//将encodedResource 加入到资源threadLocal-set 里，加入失败 说明当前encodedResource已经加载过来 不能重复加载
		//抛出异常处理
		if (!currentResources.add(encodedResource)) {
			throw new BeanDefinitionStoreException(
					"Detected cyclic loading of " + encodedResource + " - check your import definitions!");
		}
		//拿到encodedResource 包装的 输入流对象
		try (InputStream inputStream = encodedResource.getResource().getInputStream()) {
			//接下来要用xmlsax 解析器 解析xml  所以包装成 InputSource
			...
			//currentResources 表示的是当前线程正在加载的resource
			//加载完后 就将encodedResource 资源移除
			currentResources.remove(encodedResource);
			//注意 这里防止threadLocal内存泄漏
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			//将 source 转化为 程序可识别的Document 对象
			Document doc = doLoadDocument(inputSource, resource);
			//将Document 解析成bd 注册到bf 中，返回 本次新注册到bf 中bd的数量
	 ...
	 * 将 inputSource 转成 程序识别的 document 对象
	 */
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		//1、EntityResolver 获取xml 的 xsd 和dtd 的文件 DelegatingEntityResolver
		//2、验证模式是怎么获取的
	...
	protected int getValidationModeForResource(Resource resource) {
		//获取默认 的validationMode
		int validationModeToUse = getValidationMode();
		//条件成立 说明set 设置过 默认值，一般不会这么做 都是使用自动检测
		//VALIDATION_AUTO 自动检测
	...
	protected int detectValidationMode(Resource resource) {
		...
		try {
			//detectValidationMode 获取验证模式 返回xsd 或 dtd
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		//创建一个 BeanDefinitionDocumentReader 每一个 Document 都会创建一个BeanDefinitionDocumentReader 对象
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		//getRegistry 返回程序创建的 beanFactory 实例， countBefore是解析 该doc 之前 bf 中已有的 bd数量
		int countBefore = getRegistry().getBeanDefinitionCount();
		//解析 doc 并且注册达到bf中
		//xmlReaderContext 包含主要参数 当前 this -> xmlBeanDefinitionReader -> bf 通过这个路径就可以 注册到bf 中
